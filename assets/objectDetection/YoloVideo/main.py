...
from carMoving.car_movement_detector import CarMovementDetector
from colorDetection.color_detection import detect_traffic_light_color, describe_traffic_light_color
from textToSpeech.text_to_speech import speak
...
# Variáveis para armazenar o estado anterior do semáforo
last_traffic_light_color = None
last_speech_time = time.time()
speech_interval = 5  # intervalo de 5 segundos entre falas

# Inicializar o detector de movimento dos carros
car_movement_detector = CarMovementDetector()
    ...
    try:
        ...
        for r in results:
            ...
            for box in boxes:
                ...
                if class_name in relevant_objects:
                    class_name_pt = class_translation.get(class_name, class_name)

                    if class_name == 'car':
                        # Armazena a posição do carro
                        current_car_positions[(x1, y1, x2, y2)] = (x1, y1)
                        # Desenhe uma caixa delimitadora ao redor do carro
                        cv2.rectangle(img, (x1, y1), (x2, y2), (0, 255, 0), 2)

                    if class_name == 'traffic light':
                        # Desenhe uma caixa delimitadora ao redor do semáforo
                        cv2.rectangle(img, (x1, y1), (x2, y2), (255, 0, 0), 2)

                        roi = img[y1:y2, x1:x2]
                        traffic_light_color = detect_traffic_light_color(roi)
                        description = describe_traffic_light_color(traffic_light_color, "em frente")
                        if traffic_light_color != last_traffic_light_color:
                            message = f"Semáforo com sinal {traffic_light_color}."
                            if time.time() - last_speech_time > speech_interval:
                                speak_thread = threading.Thread(target=speak, args=(message,))
                                speak_thread.start()
                                last_speech_time = time.time()
        ...
        # Atualiza as posições dos carros no detector
        car_movement_detector.update_positions(current_car_positions)

        # Verifica se todos os carros estão parados e se o semáforo está verde
        if car_movement_detector.are_all_cars_stopped() and last_traffic_light_color == 'vermelho':
            message = 'Todos os carros estão parados. Pode atravessar com segurança.'
            if time.time() - last_speech_time > speech_interval:
                speak_thread = threading.Thread(target=speak, args=(message,))
                speak_thread.start()
                last_speech_time = time.time()

        # Exibe a imagem ajustada ao tamanho da janela
        cv2.namedWindow("Imagem", cv2.WINDOW_NORMAL)
        cv2.resizeWindow("Imagem", width, height)
